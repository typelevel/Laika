...
class XHTMLRendererSpec extends CatsEffectSuite with ParagraphCompanionShortcuts {
  ...
  trait DocBuilder {
    ...
    def markupDoc(num: Int, path: Path = Root): Document =
      Document(path / ("doc" + num), RootElement(p("Doc" + num)))
  ...
  test("paragraph containing a citation link with an epub:type attribute") {
    ...
    val html =
      """<p>some <a class="citation" href="#ref" epub:type="noteref">[label]</a> span</p>"""
  ...
  test("translate to external URL when an internal link is not defined for EPUB as a target") {
    ...
    assertEquals(
      defaultRenderer.render(elem, Root / "doc", translator, StyleDeclarationSet.empty),
      Right(html)
    )
  }
