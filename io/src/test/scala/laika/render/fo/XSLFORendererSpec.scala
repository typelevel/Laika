...
class XSLFORendererSpec extends FunSuite with ParagraphCompanionShortcuts with TestSourceBuilders {
  ...
  test("render a bullet list with forced paragraphs as list items the same way as normal paragraphs") {
  ...
  test("render an enumerated list with forced paragraphs as list items the same way as normal paragraphs") {
  ...
  test("render a title containing emphasized text") {
    ...
    val fo = """<fo:block color="#007c99" font-family="sans-serif" font-size="24pt" font-weight="bold" keep-with-next="always" space-after="6mm" space-before="0mm">some <fo:inline font-style="italic">em</fo:inline> text</fo:block>"""
    run(elem, fo)
  }

  test("render a title containing a section number") {
    val elem = Title(SectionNumber(Seq(1,2,3)), Text("Title"))
    val fo = """<fo:block color="#007c99" font-family="sans-serif" font-size="24pt" font-weight="bold" keep-with-next="always" space-after="6mm" space-before="0mm">1.2.3 Title</fo:block>"""
    run(elem, fo)
  }

  test("render a paragraph containing emphasized text") {
    val elem = p(Text("some "), Emphasized("em"), Text(" text"))
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:inline font-style="italic">em</fo:inline> text</fo:block>"""
    run(elem, fo)
  }

  test("render a paragraph containing strong text") {
    val elem = p(Text("some "), Strong("strong"), Text(" text"))
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:inline font-weight="bold">strong</fo:inline> text</fo:block>"""
    run(elem, fo)
  }

  test("render a paragraph containing a deleted span") {
    val elem = p(Text("some "), Deleted(Seq(Text("deleted"))), Text(" text"))
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:inline text-decoration="line-through">deleted</fo:inline> text</fo:block>"""
    run(elem, fo)
  }

  test("render a paragraph containing an inserted span") {
    val elem = p(Text("some "), Inserted("inserted"), Text(" text"))
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:inline text-decoration="underline">inserted</fo:inline> text</fo:block>"""
    run(elem, fo)
  }
  ...
  test("render a paragraph containing only an image centered") {
    ...
    val fo = s"""<fo:block space-after="3mm" text-align="center"><fo:external-graphic content-width="scale-down-to-fit" height="auto" scaling="uniform" src="/foo.jpg" width="85%"/></fo:block>"""
    run(elem, fo)
  }
  
  test("render a paragraph containing an image without title") {
    val elem = p(Text("some "), Image(imageTarget, alt = Some("img")), Text(" span"))
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:external-graphic content-width="scale-down-to-fit" height="auto" scaling="uniform" src="/foo.jpg" width="85%"/> span</fo:block>"""
    run(elem, fo)
  }
  ...
  test("render a paragraph containing an unresolved link reference") {
    ...
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:inline font-family="monospaced" font-size="9pt">[link] [id]</fo:inline> span</fo:block>"""
    run(elem, fo)
  }
  ...
  test("render a paragraph containing an unresolved image reference") {
    ...
    val fo = s"""<fo:block $defaultParagraphStyles>some <fo:inline font-family="monospaced" font-size="9pt">![img] [id]</fo:inline> span</fo:block>"""
    run(elem, fo)
  }
