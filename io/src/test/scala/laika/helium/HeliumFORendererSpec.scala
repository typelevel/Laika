...
/** @author Jens Halm
  */
class HeliumFORendererSpec extends CatsEffectSuite with InputBuilder with ResultExtractor
    with StringOps {
  ...
  test("custom link colors") {
    ...
    val expected =
      """<fo:block font-family="Lato" font-size="10pt" line-height="1.5" space-after="3mm" text-align="justify">Text with a <fo:basic-link color="#330033" external-destination="http://foo.bar" font-weight="bold">link</fo:basic-link>.</fo:block>"""
    transformAndExtract(inputs, helium, "<fo:flow flow-name=\"xsl-region-body\">", "</fo:flow>")
      .assertEquals(expected)
  }
