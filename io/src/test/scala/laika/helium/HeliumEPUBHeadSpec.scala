...
class HeliumEPUBHeadSpec extends IOFunSuite with InputBuilder with ResultExtractor with StringOps {
  ...
  def transformAndExtractHead(inputs: Seq[(Path, String)], helium: Helium): IO[String] = transformer(helium.build).use { t =>
    for {
      ...
      res        <- IO.fromEither(resultTree.extractTidiedTagContent(Root / "name.epub.xhtml", "head")
  ...
  def transformAndExtract(inputs: Seq[(Path, String)], helium: Helium, start: String, end: String): IO[String] = transformer(helium.build).use { t =>
    for {
      resultTree <- t.fromInput(build(inputs)).toOutput(StringTreeOutput).transform
      res        <- IO.fromEither(resultTree.extractTidiedSubstring(Root / "name.epub.xhtml", start, end)
        .toRight(new RuntimeException("Missing document under test")))
    } yield res
  }
  ...
  test("metadata (language)") {
    ...
    transformAndExtract(singleDoc, helium, "<html ", ">").assertEquals("""lang="de" xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops"""")
  }
