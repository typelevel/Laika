...
private[runtime] object VersionedLinkTargets {
  ...
  private def scanTargetDirectory[F[_]: Async](
      versions: Versions,
      config: VersionScannerConfig
  ): F[Map[String, Seq[Path]]] = {
    val existingVersions =
      (versions.newerVersions ++ versions.olderVersions).map(_.pathSegment).toSet
  ...
  implicit private val versionInfoDecoder: ConfigDecoder[(Path, Seq[String])] =
  ...
  private def loadVersionInfo[F[_]: Concurrent](
      existing: BinaryInput[F]
  ): F[Map[String, Seq[Path]]] = {
    def asSync[T](res: ConfigResult[T]): F[T] =
      Concurrent[F].fromEither(res.leftMap(ConfigException.apply))
  ...
  def groupLinkTargets(
      ...
      existingVersions: Map[String, Seq[Path]]
  ): Seq[VersionedDocument] = {
