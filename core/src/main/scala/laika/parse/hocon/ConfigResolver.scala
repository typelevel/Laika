...
object ConfigResolver {
  ...
  def resolve(root: ObjectBuilderValue, 
    ...
    if (errors.nonEmpty) Left(ConfigParserErrors(errors))
      ...
      val startedObjects = mutable.Map.empty[Key, ObjectBuilderValue] // may be in progress or resolved
      ...
      def resolveValue(key: Key)(value: ConfigBuilderValue): Option[ConfigValue] = value match {
        ...
        case a: ArrayBuilderValue => Some(ArrayValue(a.values.flatMap(resolveValue(key)))) // TODO - adjust path?
      ...
      def resolveMergedValue(key: Key)(values: Seq[ConfigBuilderValue]): Option[ConfigValue] = {
        ...
        def loop(values: Seq[ConfigBuilderValue]): Option[ConfigValue] = (resolveValue(key)(values.head), values.tail) match {
        ...
        def resolvedParent(current: Key): Option[(ObjectBuilderValue, Key)] = {
          if (current.segments.isEmpty) Some((rootExpanded, current))
            ...
            sorted.lastOption.fold(resolvedParent(current.parent)) {
              case (commonPath, obv) => Some((obv, Key(current.segments.take(commonPath.segments.size + 1))))
            }
          }
        }
      ...
      def resolveConcatPart(key: Key)(part: ConcatPart): Option[ConfigValue] = part.value match {
        case SelfReference => None
        case other => resolveValue(key)(other) match {
      ...
      def concat(key: Key)(v1: ConfigValue, v2: ConfigValue): ConfigValue = {
        (v1, v2) match {
          ...
          case _ =>
            invalidPaths += ((key, s"Invalid concatenation of values. It must contain either only objects, only arrays or only simple values"))
            NullValue
        }
      }
      ...
      def resolveField(key: Key, value: ConfigBuilderValue, parent: ObjectBuilderValue): Option[ConfigValue] = {
      ...
      if (invalidPaths.nonEmpty) Left(ConfigResolverError(
        s"One or more errors resolving configuration: ${invalidPaths.map { case (key, msg) => s"'$key': $msg" }.mkString(", ")}"
      ))
      else Right(res)
    }
  }
  ...
  def mergeObjects(obj: ObjectBuilderValue): ObjectBuilderValue = {
    ...
    def resolveSelfReference(key: Key, value: ConcatValue, parent: ConfigBuilderValue): ConfigBuilderValue = {
    ...
    def mergeValues(key: Key)(cbv1: ConfigBuilderValue, cbv2: ConfigBuilderValue): ConfigBuilderValue = (cbv1, cbv2) match {
    ...
    val mergedFields = obj.values.groupBy(_.key.getOrElse(Key.root)).mapValuesStrict(_.map(_.value)).toSeq.map {
      case (key, values) => 
    ...
    val expandedFields = obj.values.map { field =>
      field.validKey.segments.toList match {
        ...
        case name :: rest =>
          field.copy(
            key = Right(key.child(name)), 
            value = expandPaths(ObjectBuilderValue(Seq(BuilderField(Right(Key(rest)), field.value))), key.child(name))
          )
        case Nil =>
          field.copy(
            key = Right(Key.root),
