...
class APISpec extends FlatSpec 
  ...
  "The API" should "support the registration of block directives" in {
    new BlockDirectives {
      ...
      (Parse as Markdown using TestDirectives parse input).content should be (root (p("arg"),p("arg1arg2")))
    }
  }
  ...
  it should "ignore the registration of block directives when run in strict mode" in {
    new BlockDirectives {
      val input = """@:oneArg arg.
        |
        |@:twoArgs arg1 name=arg2.""".stripMargin
      Parse.as(Markdown).using(TestDirectives).strict.parse(input).content should be (root (p("@:oneArg arg."),p("@:twoArgs arg1 name=arg2.")))
    }
  }
  ...
  it should "support the registration of span directives" in {
    new SpanDirectives {
      ...
      (Parse as Markdown using TestDirectives parse input).content should be (root (p("one arg two arg1arg2 three")))
    }
  }
  ...
  it should "ignore the registration of span directives when run in strict mode" in {
    new SpanDirectives {
      ...
      Parse.as(Markdown).using(TestDirectives).strict.parse(input).content should be (root (p("one @:oneArg arg. two @:twoArgs arg1 name=arg2. three")))
    }
  }
