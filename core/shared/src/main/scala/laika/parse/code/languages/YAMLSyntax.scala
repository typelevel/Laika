...
object YAMLSyntax extends SyntaxHighlighter {
  ...
  private val keyName =
    Identifier.alphaNum.withIdStartChars('_', '-', '.').withCategory(CodeCategory.AttributeName)
  ...
  private val valueLiteral = {
    ...
    val number                 =
      NumberLiteral.decimalFloat | NumberLiteral.decimalInt | NumberLiteral.octal | NumberLiteral.hex
    val boolean                =
      (literal(
        "true"
      ) | "True" | "TRUE" | "false" | "False" | "FALSE" | "on" | "On" | "ON" | "off" | "Off" | "OFF" | "yes" | "Yes" | "YES" | "no" | "No" | "NO")
    ...
    val unquotedStrLiteral =
      delimitedBy(literal(" #") | literal("\n")).acceptEOF.keepDelimiter.map(s =>
        List(CodeSpan(s, CodeCategory.StringLiteral))
      )
