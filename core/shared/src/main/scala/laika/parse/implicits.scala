...
object implicits {
  ...
  implicit class Prepend2PrefixedParserOps[T](val p: PrefixedParser[T ~ Seq[T] ~ Seq[T]])
      extends AnyVal {
  ...
  implicit class Seq3PrefixedParsersOps[T](val p: PrefixedParser[Seq[T] ~ Seq[T] ~ Seq[T]])
      extends AnyVal {
    def concat: PrefixedParser[Seq[T]] = p.map { case xs ~ ys ~ zs => xs ++ ys ++ zs }
  }

  implicit class Seq4ParsersOps[T](val p: Parser[Seq[T] ~ Seq[T] ~ Seq[T] ~ Seq[T]])
      extends AnyVal {
    def concat: Parser[Seq[T]] = p.map { case s1 ~ s2 ~ s3 ~ s4 => s1 ++ s2 ++ s3 ++ s4 }
  }

  implicit class Seq4PrefixedParsersOps[T](val p: PrefixedParser[Seq[T] ~ Seq[T] ~ Seq[T] ~ Seq[T]])
      extends AnyVal {
  ...
  implicit class PrefixedMap4Ops[A, B, C, D](val p: PrefixedParser[A ~ B ~ C ~ D]) extends AnyVal {
    ...
    def mapN[Z](f: (A, B, C, D) => Z): PrefixedParser[Z] = p.map { case a ~ b ~ c ~ d =>
      f(a, b, c, d)
    }
  ...
  implicit class Map5Ops[A, B, C, D, E](val p: Parser[A ~ B ~ C ~ D ~ E]) extends AnyVal {
    ...
    def mapN[Z](f: (A, B, C, D, E) => Z): Parser[Z] = p.map { case a ~ b ~ c ~ d ~ e =>
      f(a, b, c, d, e)
    }

  }

  implicit class PrefixedMap5Ops[A, B, C, D, E](val p: PrefixedParser[A ~ B ~ C ~ D ~ E])
      extends AnyVal {

    def mapN[Z](f: (A, B, C, D, E) => Z): PrefixedParser[Z] = p.map { case a ~ b ~ c ~ d ~ e =>
      f(a, b, c, d, e)
    }
