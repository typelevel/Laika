...
package laika.internal.parse.css
...
private[laika] object CSSParsers {
  ...
  val simpleSelectorSequence: Parser[StyleSelector] =
    ((typeSelector ~ predicate.rep).concat | predicate.rep.min(1)).map(preds =>
      StyleSelector(preds.toSet)
    )

  /** Parses a single selector.
    */
  val selector: Parser[StyleSelector] =
    simpleSelectorSequence ~ (combinator ~ simpleSelectorSequence).rep ^^ { case sel ~ sels =>
      sels.foldLeft(sel) {
  ...
  /** Parses a single style within a declaration.
    */
  val style: Parser[Style] =
    ((styleRefName <~ ws ~ ":" ~ ws) ~ (styleValue <~ wsOrNl)).mapN(Style.apply)
  ...
  lazy val styleDeclarationSet: Parser[Set[StyleDeclaration]] = {
    ...
    (wsOrNl ~ comment.rep ~> (styleDeclarations <~ wsOrNl ~ comment.rep).rep).map {
      _.flatten.zipWithIndex.map({ case (decl, pos) =>
        decl.increaseOrderBy(pos)
      }).toSet
    }
  }
