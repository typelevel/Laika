...
object InlineParsers {
  ...
  lazy val footnoteRef: SpanParserBuilder = SpanParser.standalone {
    (markupStart("[", "]_") ~> footnoteLabel <~ markupEnd("]_")).withCursor.map { case (label, source) => 
  ...
  lazy val citationRef: SpanParserBuilder = SpanParser.standalone {
    (markupStart("[", "]_") ~> simpleRefName <~ markupEnd("]_")).withCursor.map { case (label, source) =>
  ...
  lazy val interpretedTextWithRolePrefix: SpanParserBuilder = SpanParser.recursive { recParsers =>
    ((markupStart(":", ":") ~> simpleRefName) ~ (":`" ~> recParsers.escapedText(delimitedBy(markupEnd("`")).nonEmpty))).withCursor.map
      { case (role ~ text, src) => InterpretedText(role, text, src) }
  }
  ...
  def interpretedTextWithRoleSuffix (defaultTextRole: String): SpanParserBuilder = SpanParser.recursive { recParsers =>
    ...
    (textParser ~ roleSuffix).withCursor.map { 
      case (text ~ role, src) => InterpretedText(role.getOrElse(defaultTextRole), text, src) 
    }
  }.withLowPrecedence
