...
class ElementTraversalSpec extends FunSuite with ParagraphCompanionShortcuts {
  ...
  test("select all elements that satisfy the predicate") {
    ...
    assertEquals(
      rootElem.select { case Paragraph(_, _) => true; case _ => false },
  ...
  test("select the elements in depth-first order") {
    ...
    assertEquals(
      rootElem.select { case QuotedBlock(_, _, _) => true; case _ => false },
      List(
        QuotedBlock("a"),
        QuotedBlock(QuotedBlock("a")),
        QuotedBlock("b")
      )
    )
  }
  ...
  test("select elements which are not part of the content collection of a container") {
    ...
    assertEquals(
      rootElem.select { case Header(_, _, _) => true; case _ => false },
      List(Header(1, "Title"))
    )
  }
