...
class RewriteSpec extends FunSuite with ParagraphCompanionShortcuts {
  ...
  test("return a new instance for a branch in the document tree that contains one or more modified children") {
  ...
  test("rewrite a span container") {
    ...
    val after = before.rewriteSpans { case Emphasized(Seq(Text("b",_)),_) => Replace(Strong("x")) }
  ...
  test("rewrite text in table cells") {
    ...
    val expected = RootElement(Table(Row(BodyCell("x"), BodyCell("b")), Row(BodyCell("x"), BodyCell("c"))))
    val after = before.rewriteSpans { case Text("a", _) => Replace(Text("x")) }
    assertEquals(after, expected)
  }
