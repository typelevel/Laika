...
class RewriteRulesSpec extends FunSuite with ParagraphCompanionShortcuts with TestSourceBuilders {
  ...
  def runRoot (input: RootElement, expected: RootElement): Unit = assertEquals(rewritten(input), Right(expected))
  ...
  test("footnote rules - retain a group of footnotes with a mix of explicit numeric and autonumber labels") {
  ...
  test("footnote rules - retain a group of footnotes with a mix of explicit numeric, autonumber and autonumber-labeled footnotes") {
  ...
  test("footnote rules - replace references with unresolvable autonumber or numeric labels with invalid spans") {
  ...
  test("link id refs - interpret internal link definitions as external when they point upwards beyond the virtual root") {
    val rootElem = RootElement(p(linkIdRef()), LinkDefinition("name", InternalTarget(RelativePath.parse("../../foo.md#ref"))))
  ...
  test("link id refs - resolve anonymous internal link definitions") {
    ...
    val resolved = RootElement(p(intLink(RelativePath.parse("foo.md#ref")), intLink(RelativePath.parse("bar.md#ref")))) 
    runRoot(rootElem, resolved)
  }
  ...
  test("link id refs - resolve references when some parent element also gets rewritten") {
    ...
    val resolved = RootElement(Title(List(Text("text "), extLink("http://foo/")), Id("text-text") + Style.title))
    runRoot(rootElem, resolved)
  }
  ...
  object IdRefs {
    ...
    private val linkConfig = LinkConfig(targets = Seq(
  ...
  object InternalLinks {
    ...
    def run (ref: String, expected: Span, builder: SampleSixDocuments => SampleSixDocuments = identity): Unit = {
      val refInstance = LinkPathReference(List(Text("text")), RelativePath.parse(ref), generatedSource(s"[<$ref>]"))
      run(refInstance, expected, builder)
    }

    def run (ref: Reference, expected: Span, builder: SampleSixDocuments => SampleSixDocuments): Unit =
  ...
  test("internal links - produce an invalid span for a reference to a markup document with fewer target formats than the source") {
    val relPath = "doc-4.md#target-4"
  ...
  test("internal links - produce an invalid span for a reference to a static document with fewer target formats than the source") {
  ...
  test("internal links - allow links to missing target documents when one of the parent trees has the validateLinks flag set to false") {
    InternalLinks.run(
      "doc99.md#ref",
  ...
  test("internal links - add a restricted target format parameter for a reference to a markup document with fewer target formats than the source when siteBaseURL is defined") {
    val relPath = "doc-4.md#target-4"
    InternalLinks.run(
      relPath,
  ...
  test("internal links - add a restricted target format parameter for a reference to a static document with fewer target formats than the source when siteBaseURL is defined") {
    val relPath = "../static/doc.html"
    val absPath = Root / "static" / "doc.html"
    InternalLinks.run(
      relPath,
  ...
  test("internal links - resolve a link id reference to a header with a duplicate id by precedence") {
  ...
  test("image id refs - resolve external link references") {
    ...
    val expected = RootElement(p(Image(ExternalTarget("http://foo.com/bar.jpg"), alt = Some("text"))))
    runRoot(rootElem, expected)
  }
  ...
  test("image id refs - resolve internal link references") {
    ...
    val expected = RootElement(p(Image(InternalTarget(Root / "foo.jpg").relativeTo(refPath), alt = Some("text"))))
    runRoot(rootElem, expected)
  }
  ...
  test("decorated headers - do not create title nodes in the default configuration for orphan documents") {
    val rootElem = RootElement(
      DecoratedHeader(Underline('#'), List(Text("Title")), GeneratedSource),
      DecoratedHeader(Underline('#'), List(Text("Header 1")), GeneratedSource),
      DecoratedHeader(Underline('#'), List(Text("Header 2")), GeneratedSource)
