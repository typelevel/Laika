...
/** @author Jens Halm
  */
class StandardTextRolesSpec extends FunSuite with ParagraphCompanionShortcuts
  ...
  def run(input: String, expected: Span)(implicit loc: munit.Location): Unit =
    run(input, p(Text("some "), expected))
  ...
  test("raw - with format and style options") {
    ...
    val expected = RootElement(
      p(Text("some "), RawContent(NonEmptySet.of("AML", "BML", "CML"), "text", Styles("foo")))
    )
  ...
  test("raw - disabled by default") {
    ...
    run(
      ...
      p(
        Text("some "),
        InvalidSpan("unknown text role: foo", source(":foo:`text`", input, defaultPath))
      )
    )
  }
