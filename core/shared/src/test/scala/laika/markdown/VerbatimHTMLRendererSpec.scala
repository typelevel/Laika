...
class VerbatimHTMLRendererSpec extends FunSuite
  ...
  test("render a script element with attributes with content unescaped") {
    ...
    run(
      elem,
      "<p>some <script type=\"text/javascript\" defer> var x = 'foo'; </script> &amp; text</p>"
    )
  }
  ...
  test("render an HTML start tag with two attributes unescaped") {
    val elem =
      p(Text("some "), startTag("hr", ("foo", Text("bar")), ("bar", Text("foo"))), Text(" & text"))
  ...
  test("render an HTML element with one attribute unescaped") {
    val elem = p(
      Text("some "),
      element(startTag("span", ("foo", Text("bar"))), Text("inner")),
      Text(" & text")
    )
  ...
  test("render a <pre> element without indentation") {
    val elem = QuotedBlock(
      p("1st paragraph"),
      p(Text("some "), element(startTag("pre"), Text("Line1\nLine2")), Text(" text")),
      p("3rd paragraph")
    )
    val html = """<blockquote>
                 |  <p>1st paragraph</p>
