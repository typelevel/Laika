...
trait RenderPhaseRewrite {
  ...
  def rewrite (parser: MarkupParser, format: RenderFormat[_])(doc: Document): Either[RendererError, Document] =
    rewrite(parser.config, format)(doc)
  
  def rewrite (config: OperationConfig, format: RenderFormat[_])(doc: Document): Either[RendererError, Document] = {
    config
