...
class StandardBlockDirectivesSpec extends FunSuite with ParagraphCompanionShortcuts
  ...
  test("admonition - set id and style") {
    ...
    run(
      input,
      TitledBlock(List(Text("TITLE")), simplePars, Id("foo") + Styles("bar", "admonition"))
    )
  }
  ...
  test("attention admonition with id and style") {
    ...
    run(
      input,
      TitledBlock(List(Text("Attention!")), simplePars, Id("foo") + Styles("bar", "attention"))
    )
  }
  ...
  test("attention admonition with nested blocks and inline markup") {
    ...
    run(
      ...
      TitledBlock(
        List(Text("Attention!")),
        List(p(Text("1st "), Emphasized("Para")), QuotedBlock("2nd Para")),
        Styles("attention")
      )
    )
  }
  ...
  test("sidebar - set id and subtitle") {
    ...
    run(
      ...
      TitledBlock(
        List(Text("TITLE")),
        Paragraph(List(Text("some "), Emphasized("text")), RstStyle.subtitle) :: simplePars,
  ...
  test("rubric - set id and styles") {
    ...
    run(
      input,
      Paragraph(List(Text("some "), Emphasized("text")), Id("foo") + Styles("bar", "rubric"))
    )
  }
  ...
  test("epigraph - multiple lines") {
    ...
    run(
      input,
      QuotedBlock(
        List(p(Text("some "), Emphasized("text"), Text("\nsome more"))),
        Nil,
        Styles("epigraph")
      )
    )
  ...
  test("epigraph - with attributions") {
    ...
    run(
      ...
      QuotedBlock(
        List(p(Text("some "), Emphasized("text"), Text("\nsome more"))),
        List(Text("attr", Style.attribution)),
        Styles("epigraph")
      )
    )
  }
  ...
  test("parsed-literal - set id and style") {
    ...
    run(
      input,
      ParsedLiteralBlock(
        List(Text("some "), Emphasized("text"), Text("\nsome more")),
        Id("foo") + Styles("bar")
      )
    )
  }
  ...
  test("parsed-literal - multiple lines separated by spaces, preserving indentation") {
    ...
    run(
      input,
      ParsedLiteralBlock(List(Text("some "), Emphasized("text"), Text("\nsome more\n\n indented")))
    )
  }
  ...
  test("grid table with caption") {
    ...
    run(
      ...
      Table(Row(BodyCell("a"), BodyCell("b")), Row(BodyCell("c"), BodyCell("d"))).copy(caption =
        Caption(List(Emphasized("caption")))
      )
    )
  }
  ...
  test("figure with a caption") {
    ...
    run(
      input,
      Figure(Image(resolvedImageTarget), List(Text("This is the "), Emphasized("caption")), Nil)
    )
  }
  ...
  test("figure with a caption and a legend") {
    ...
    run(
      input,
      Figure(
        Image(resolvedImageTarget),
        List(Text("This is the "), Emphasized("caption")),
        List(p("And this is the legend"))
      )
    )
  }
  ...
  test("meta - creates config entries in the document instance") {
    ...
    val res      = parser.parse(input).flatMap(_.config.get[ConfigValue]("meta"))
  ...
  test("sectnum - creates config entries in the document instance") {
    ...
    val res      = parser.parse(input).flatMap(_.config.get[ConfigValue](LaikaKeys.autonumbering))
    assertEquals(res, Right(expected))
  }
  ...
  test("raw - support raw input with one format") {
    ...
    assertEquals(
      res,
      Right(RootElement(RawContent(NonEmptySet.one("format"), "some input\n\nsome more")))
    )
  }
