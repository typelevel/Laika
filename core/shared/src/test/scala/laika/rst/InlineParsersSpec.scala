...
class InlineParsersSpec extends AnyFlatSpec 
  ...
  it should "parse an internal phrase link with text and url" in {
    ...
    val spanSeq = List(LinkPathReference(Seq(Text("link")), relPath, linkSrc), LinkDefinition("link", InternalTarget(Root, relPath)))
  ...
  it should "parse an internal phrase link with only a url" in {
    ...
    val spanSeq = List(LinkPathReference(Seq(Text("../foo/bar.rst#ref")), relPath, linkSrc), LinkDefinition("../foo/bar.rst#ref", InternalTarget(Root, relPath)))
