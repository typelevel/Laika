...
class BlockDirectiveAPISpec extends FunSuite
  ...
  trait BlockParser extends RenderPhaseRewrite {
    ...
    lazy val directiveSupport: ParserBundle = DirectiveSupport.withDirectives(
      Seq(directive, StandardDirectives.blockStyle),
      Nil,
      Nil,
      Nil
    ).parsers
    ...
    def invalid(fragment: String, error: String): InvalidBlock =
      InvalidBlock(error, source(fragment, input))
