...
class TemplateDirectiveAPISpec extends FunSuite with TestSourceBuilders {
  ...
  trait TemplateParser {
    ...
    def run(input: String, result: TemplateSpan)(implicit loc: munit.Location): Unit =
      assertEquals(
        defaultParser.parse(input).toEither,
        Right(
          TemplateRoot(
            TemplateString("aa "),
            result,
  ...
  test(
    "parse a full directive spec with all elements present with attributes spanning three lines"
  ) {
    new FullDirectiveSpec with TemplateParser {
      val body = TemplateSpanSequence("foo:str:11", " 1 ", "value", " 2 ")
      run("aa @:dir(foo,4) { \nstrAttr=str\nintAttr=7\n } 1 ${ref} 2 @:@ bb", body)
  ...
  test(
    "detect a full directive spec with all positional attributes and the required body missing"
  ) {
