...
private[preview] class SourceChangeWatcher[F[_]: Async](
  ...
  private def scanDirectory(dir: FilePath): F[List[FilePath]] =
  ...
  val init: F[Unit] = {
    ...
    groupedInputs
      .flatMap {
        _.map { case (parent, children) =>
          registerRoot(parent, children)
        }
          .toList
          .sequence
...
private[preview] object SourceChangeWatcher {
  ...
  def scheduledTaskResource[F[_]: Temporal](
      task: F[Unit],
  ...
  def create[F[_]: Async](
    ...
    if (inputs.isEmpty) Resource.unit
      ...
      Resource
        .make(Async[F].delay(FileSystems.getDefault.newWatchService))(service =>
          Async[F].delay(service.close())
        )
        .evalMap(s => Async[F].ref(Map.empty[WatchKey, ObservedTarget]).map((s, _)))
        .flatMap { case (service, targetMap) =>
          val watcher = new SourceChangeWatcher[F](
            service,
            targetMap,
            inputs,
            update,
