...
trait BlockParsers extends MarkupParsers {
   ...
   *  text.
   */
  def parseDocument (reader: Reader[Char], path: Path): Document = new Document(path, Nil, DocumentInfo(), parseMarkup(root, reader), None, null) // TODO - fully populate title, info, config
