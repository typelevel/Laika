...
class RewriteRulesSpec extends FlatSpec 
  ...
  it should "resolve references when some parent element also gets rewritten" in {
    ...
    rewritten (document) should be (doc(Header(1, List(txt("text1"), extLink("http://foo/")), Id("header") + Styles("title"))))
  }
  ...
  "The rewrite rules for header ids" should "retain the id of a header" in {
    ...
    rewritten (document) should be (doc(Header(1, List(Text("text")), Id("header") + Styles("title"))))
  }
  ...
  it should "append numbers to duplicate ids" in {
    ...
    rewritten (document) should be (doc(Section(Header(1, List(Text("text1")), Id("header") + Styles("section")), Nil), 
