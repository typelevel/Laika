...
* `laika:packageSite`  
  ...
  directory. Depends on the `laika:site` task.
...
* `laika:generate`
  ...
  For example `laika:generate html pdf` generates an HTML site as well
  ...
  (`laika:html`, `laika:pdf`, `laika:xslfo`, `laika:prettyPrint`)
...
* `includeAPI` - `Boolean` - default `false`  
  Indicates whether generated scaladoc APIs should be included when running the `laika:site` task. 
  The folder for the API can be set with `target in copyAPI` (default `target/docs/site/api`).
  
* `includePDF` - `Boolean` - default `false`
  Indicates whether a generated PDF file should be included when running the `laika:site` task.
...
The `foRenderers` setting is of type `Seq[FOWriter => RenderFunction]` and can be used
...
Finally the `prettyPrintRenderers` setting is of type `Seq[TextWriter => RenderFunction]`
