...
and run `laika:site` from within sbt to generate the site. See the following
...
* `laika:site`: Processes all files from the source directories and renders the 
  result to `target/docs/site` (unless overridden). 

* `laika:packageSite`: Packages the site into a zip file inside the `target/docs`
  directory.

* `laika:clean`: Deletes the contents of Laika's target directory only. Alternatively
...
* `sourceDirectories` (`Seq[File]` - default `Seq("src/docs")`): Specifies one or more
...
* `target` (`File` - default `target/docs`): Specifies the directory where the plugin
...
* `excludeFilter` (`FileFilter` - default `HiddenFileFilter`): Specifies files to exclude from processing.
...
* `includeAPI` (`Boolean` - default `false`): Indicates whether generated scaladoc APIs should be
  copied to the site. The folder for the API can be set with `target in copyAPI` (default `target/docs/site/api`).

* `encoding` (`String` - default `UTF-8`): Specifies the character encoding for input and output.

* `strict` (`Boolean` - default `false`): Indicates whether all Laika-specific extensions to the 
...
* `rawContent` (`Boolean` - default `false`): Indicates whether raw content like embedded verbatim
  ...
  `raw` directive which allows to embed content any target format (although in the context of this plugin
  only HTML would make sense). 

* `logMessageLevel` (`Option[MessageLevel]` - default `Some(Warning)`): Specifies the log level for
  ...
  case you want to fix these issues and these logs help with that. `Info` and `Debug` levels are
  currently not used by Laika.

* `renderMessageLevel` (`Option[MessageLevel]` - default `None`): In addition to just logging errors,
...
* `parallel` (`Boolean` - default `true`): Indicates whether the parsers and renderers should be 
...
For more details on this directive type see [Implementing a Directive].
